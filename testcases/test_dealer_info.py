...
class dealerInfoCheck(unittest.TestCase):
    maxDiff = None
    def setUp(self):
        self.s = login()
    ...
    successData = loadTestData("dealer_info_success.xlsx")
    @parameterized.expand(successData)
    def test_info_success(self, _, data, expect):
    ...
    failData = loadTestData("dealer_info_fail.xlsx")
    @parameterized.expand(failData)
    def test_info_fail(self, _, data, expect):
        datas = json.dumps(json.loads(data)).encode('utf-8')
        expects = eval(expect)
        r = self.s.post(self.baseurl, datas)
        result = r.json()
